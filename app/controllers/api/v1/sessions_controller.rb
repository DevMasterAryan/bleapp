...
class Api::V1::SessionsController < ApplicationController
	...
	def verify_otp
		...
		if @user.present?
		    ...
		    @user.save
			render json: {responseCode: 200, responseMessage: "Login successfully.",access_token: @user.access_token,mop: @user&.billings&.last&.method_of_payment,site_display_name: @user&.billings&.last&.session&.device&.site_display_name, site_name: @user&.billings&.last&.session&.device&.site_display_name? ? @user&.billings&.last&.session&.device&.location&.name : "", billing_id: @user&.billings&.last&.id&.as_json["$oid"], promotion: @user.promotion }
		else
