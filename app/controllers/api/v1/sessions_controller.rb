	...
	def social_login
	  	begin	  	
			...
			if @user.present?
			   ...
			   if @user.billings.last.present? and (DateTime.current < @user.billings.last.usage_end_ts)
			     return render json: {responseCode: 200, responseMessage: "Login successfully." ,access_token: @user.access_token, first_name: @user&.first_name, last_name: @user.last_name, image: @user&.image&.url,provider_id: @social&.provider_id, start_time: @user&.billings&.last&.usage_start_ts&.to_i,end_time: @user&.billings&.last&.usage_end_ts&.to_i || "", credit: @user&.credit, mop: @user&.billings&.last&.method_of_payment&.downcase,site_display_name: @user&.billings&.last&.session&.device&.site_display_name, site_name: @user&.billings&.last&.session&.device&.site_display_name? ? @user&.billings&.last&.session&.device&.site&.site_name : "", billing_id: @user&.billings&.last&.id&.as_json["$oid"], promotion: @user.promotion_count, user_id: @user.id.as_json["$oid"], remaining_time:  (@user&.billings&.last&.usage_end_ts.to_f - DateTime.current.to_f)|| 0 }
			   else
